...
- Compilers: `always_inline` on `probe` and `do_place_element` kept (17–20% of a build in a one-map TU; costs the score, which is a 90-TU binary); clang's insert split = 15 instr/insert call boundary, PGO removes it, six source changes did not. `probe_past_home`'s return shape swept and closed (#267): dropping `value_idx` costs 4.1% of the string find's instructions under clang, `found` as a sentinel 1.1%, the integer find unmoved in every cell -- clang already packs the twelve bytes into the two return registers, so there is nothing to save. `try_emplace`'s hit path (#305): the out-of-line hit uses 15 GPRs, so its six-register prologue cannot be shrink-wrapped; placement out of line, `always_inline` outward, fewer values across the probe and `flatten` on the caller all null or worse (clang 72.0 instr/hit, 77.0 flattened; gcc 45.1). "probe_result's shape", "call boundary", "attacked from the hit side"
- Maximum load factor (#306): 0.75/0.85/0.875/0.9 against 0.8 at `-p 50`, both compilers agree to 0.1%: 1.008 / 0.985 / 0.974 / 0.960 of the geomean, index 1.067 / 0.941 / 0.914 / 0.889 of 0.8's. Kept at 0.8; in cache only, past-cache not swept. "maximum load factor swept"
- Teardown order (2026-09-26): `segmented_vector` destroys and frees last first; forward order made glibc trim 106-131 MB per 1M-entry teardown that the next build faulted back in (warm build 1.8-2.3x strings, 2.6-3.2x owning values). Price: freed memory stays resident until reused or `malloc_trim`, as with `std::unordered_map`. glibc only. "Tearing a segmented_map down in reverse"
...
- **Memory has two answers and both are measured**: `scripts/ab/max_rss.h` (resident pages) and `scripts/ab/count_alloc.h` (bytes asked for), used by `bench_readme` and `maps.cpp`. Resident over asked sorts by family: node 0.85-1.04x, dense 1.05-1.55x, flat 1.50-1.97x, so the verdict inverts between them (absl asks 26.4 B/entry and occupies 48.8; this map asks 31.8 and occupies 42.0). Quote both or say which. **Fork per measurement** in both -- the second fill reuses the grown arena and reads too low, and a counted pass run in-process makes every *later* RSS cell read near zero. **Subtract the instrument's floor**: forking + `clear_refs` + reading `/proc/self/status` costs ~128 KB, constant, which is 0.5% at 500k entries and the whole answer at 1000; deterministic within a process, ~20 KB residual after subtraction, **no thousand-entry column from it**. Interpose `malloc`/`calloc`/`realloc`/`mmap` **and `aligned_alloc`/`posix_memalign`** (ihtab read 8.3 B/entry against 35.3 without them), charging `malloc_usable_size` + 8. Never link the counter into a binary that measures time. Huge pages are counted correctly and were checked. **A benchmark that allocates needs a volatile sink** -- clang elides `vector`+`memset`+one read outright, and a touched 16 MB measured as an empty closure. "`max_rss::of` was charging every map 128 KB"
- Rehash pipeline, the size axis below 200000 (2026-09-15): `rehash(0)` on its own wants it at nearly every size for a string key (up to 1.53x) and, for an integer key, up to 61 KB of index and from 344 KB on but **not between** -- 0.953-0.971 from 86 to 243 KB on both compilers. End to end it costs a clang build nothing at any size (worst cell 0.979) and costs a gcc integer build 1.7-4.2% from 43 KB to 486 KB. The ring is 11 instr/element flat: -8% cycles at 21 KB, +8% at 122 KB, -6% at 2.7 MB. Kept, no gate: the losing band is bounded on both sides, so one index-bytes threshold cannot express it, and separating it needs to know whether the key's hash is expensive. **The harness had to be pinned first** -- two timed functions in one TU moved the pipelined side 11% and turned a 4% dip into 13%, with a byte-identical-binary check proving it was the inliner and not layout; one mode per TU, and the same sweep twice agrees to 1.5%. `scripts/ab/rehash_size.sh`, `scripts/ab/rehash_plain.patch`. "The rehash pipeline below 200000 entries"
- Still open: a probe-length statistics facility -- but `scripts/ab/probe_length.cpp` already answers it out of tree and says the counter "does not belong in the header, where it would be in everybody's lookup".
