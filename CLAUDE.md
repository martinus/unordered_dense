...
- Chasing gcc's codegen: g++ 13 and clang 18 are at parity on the (checksum-verified) workloads except a real ~3.7 % gcc win on the string find benchmark, caused by gcc fully inlining the hash into the find loop while clang keeps an outlined call. Not closable from portable source: force-inline, `__attribute__((preserve_most))`, and a lower-register-pressure 4-lane hash loop all tied or regressed (handoff doc E1/E9/E10). Also: benchmark harnesses whose results aren't consumed get dead-code-eliminated by gcc (not clang) — only trust harnesses that verify their result, like `scripts/microbench/micro.cpp`.
