...
- **Peak memory as resident pages, not bytes requested** (`scripts/ab/max_rss.h`, used by `bench_readme` and `maps.cpp`): a doubling flat map leaves every superseded array resident in glibc's arena, worth 1.04-1.28x where counted bytes say 0.96-1.10x; a node map's two numbers agree to 1%. Fork per measurement, or the second fill reuses the grown arena and reads too low; reset `VmHWM` via `/proc/self/clear_refs`. Huge pages are counted correctly and were checked (`AnonHugePages` 28 MB, VmHWM 2 MB above 4 KB pages). One build is enough -- it is deterministic to the page. "Seventeen maps in their own default configurations"
