...
- Values: narrower index / tiny pointers ~9% memory, no speed (#229); slot back-pointer re-tested across the cache boundary and closed (#266): string erase *by key* 0.88-0.92 at every size from 50k to 4M, and a wash on churn because the insert tax is the same size -- score 0.987 clang / 0.973 gcc, +18.6% memory, `scripts/ab/back_pointer.patch`; distance nibbles 0.959; growth < 2 is a knob; not zeroing the index 1.7% and UB. "tiny pointers", "back-pointer"
...
- Still open: #267, #271, #272; a probe-length statistics facility; the rehash has never been measured below cache.
