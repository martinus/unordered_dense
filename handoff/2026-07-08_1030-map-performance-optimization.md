...
| E9 | `__attribute__((preserve_most))` on `wyhash::hash` | clang keeps the hash outlined (~15 instr call overhead/find vs gcc's full inline, see the gcc note below); shift register saving out of the caller's find loop | findstr +1.4 % worse (4:2 against) | the hash body clobbers ~10 registers, so the callee-side saves cost more than the caller-side spills they replace |
| E10 | 4-lane 64 B bulk loop instead of 6-lane 96 B (same mul count for 200 B inputs, 2 fewer live registers) | reduce the register pressure that makes clang's inlining/spilling of the hash unprofitable | findstr 3:3 exact tie (+0.4 %), iestr 3:3 (−0.1 %) | register pressure isn't the binding constraint in the outlined function; in isolation the 4-lane variant had better throughput but worse latency, and in map context they cancel out |
...
1. **GCC vs clang codegen diff — DONE, mostly a retraction.** An earlier claim that gcc compiles `find64` 2.5×/4.5× better was **wrong**: it came from an unverified scratchpad copy of the harness whose result chain was dead, so gcc dead-code-eliminated the finds/sums (clang didn't). On the committed, checksum-verified harness the compilers are at parity: find64 497 M (clang) vs 515 M (gcc) Ir, 118 vs 122 ms; it64 7.3 vs 8.2 ms; ie64/iestr ±3 %. **One real residual**: gcc wins `findstr` by ~3.7 % (6:0 pairs, 347.5 vs 334.7 ms). Asm diff shows why: gcc fully inlines wyhash into the find loop with loop-wide register allocation; clang keeps an outlined call (6 push + 6 pop + call/ret + caller spills ≈ 15–20 instr ≈ 3.6 cycles/find). Attempts to close it in portable source all failed: force-inline (E1), `preserve_most` (E9), 4-lane loop to cut register pressure (E10). Conclusion: it's an inlining/regalloc quality gap, not reachable from source; don't burn time here again. Methodology lesson: **never benchmark a harness whose results aren't consumed** — the checksum verification in `scripts/microbench/micro.cpp` exists exactly for this.
